...
class HackerModeInstaller:
        ...
        if show_output and install_message:
            if os.path.isdir(Variables.TOOL_INSTALL_PATH):
                if all([modules, packages]):
                    print(f"# {YELLOW}AmineMode installed successfully but there is some issues.{NORMAL}")
                else:
                    print(f"# {GREEN}AmineMode installed successfully.{NORMAL}")
            else:
    ...
    def update(self):
        if not Config.get('actions', 'DEBUG', cast=bool, default=False):
            hackermode_command_line_path = os.environ.get("_").split("bin/")[0] + "bin/AmineMode"
            if os.path.exists(hackermode_command_line_path):
                os.remove(hackermode_command_line_path)
            os.system(
                f'curl https://raw.githubusercontent.com/Arab-developers/AmineMode/main/install.sh > AmineMode.install 2> .AmineMode-install.log && {Variables.SHELL_COMMAND} AmineMode.install')
        else:
            print("# can't update in the DEUBG mode!")

    def add_shortcut(self):
        # add AmineMode shortcut...
        try:
        ...
        if status in ("y", "yes", "ok", "yep"):
            ...
            if os.path.exists(tool_path):
                try:
                    ...
                    if show_message:
                        errors += 1
                        print("# cannot remove AmineMode shortcut!")
                try:
